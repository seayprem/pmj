...
<body>
  ...
  <div class="main-container d-flex">
    ...
    <div class="dashboard-content px-3 pt-4">
      <h2 class="fs-5"> รายการวัสดุสำนักงานที่อนุมัติ</h2>
      <hr>
      <!-- START DATE SELECT  -->
      <form action="lists_accept.php" method="POST">
      <div class="mb-3">
        <div class="row">
      ...
      <tbody>
          ...
          if(isset($_POST['time_select'])) {
            ...
            if(empty($date_start) && empty($time_start) && empty($date_end) && empty($time_end)) {
              ...
              $sql = "SELECT * FROM `transfer` INNER JOIN status ON transfer.stat_id = status.stat_id INNER JOIN employees ON transfer.emp_id = employees.emp_id WHERE status.stat_status = 2 AND DATE(t_datetime) BETWEEN '$date_start' AND '$date_end'";
            } else {
              $sql = "SELECT * FROM `transfer` INNER JOIN status ON transfer.stat_id = status.stat_id INNER JOIN employees ON transfer.emp_id = employees.emp_id WHERE status.stat_status = 2 AND t_datetime BETWEEN '$date_start $time_start' AND '$date_end $time_end'";
            }
